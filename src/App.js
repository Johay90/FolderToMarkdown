    ...
    if (entry.kind === 'directory') {
      ...
      try {
        ...
        const processedEntries = await processBatch(
          ...
          (processed, total) => {
            setProgressStatus(`Scanning ${entry.name}: ${processed}/${total} items...`);
          }
        );
    ...
    try {
      ...
      if (entry.kind === 'directory') {
        ...
        try {
          ...
          const processedEntries = await processBatch(
            childEntries,
            async (childEntry) => processFileEntry(childEntry, currentPath, selectedPaths),
            (processed, total) => {
              setProgressStatus(`Processing ${entry.name}: ${processed}/${total} items...`);
            }
          );
      ...
      const getInitialStructure = async (handle) => {
        ...
        try {
          ...
          const processedEntries = await processBatch(
            ...
            (processed, total) => {
              setProgressStatus(`Scanning root folder: ${processed}/${total} items...`);
            }
          );
    ...
    try {
      ...
      const processDirectory = async (handle) => {
        ...
        const processedEntries = await processBatch(
          ...
          (processed, total) => {
            setProgressStatus(`Processing selected items: ${processed}/${total}...`);
          }
        );
